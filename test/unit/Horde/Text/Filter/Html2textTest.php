...
class Html2textTest extends TestCase
{
    ...
    public function testHtml2text($input, $expected)
    {
        ...
        $filter = Horde_Text_Filter::filter($input, 'Html2text', [
            'width' => 70,
        ]);
    ...
    public function testHtml2TextNestingLimit()
    {
        ...
        $filter = Horde_Text_Filter::filter($html, 'Html2text', ['nestingLimit' => 3]);
        $this->assertEquals('Foo', $filter);

        $filter = Horde_Text_Filter::filter($html, 'Html2text', ['nestingLimit' => 4]);
