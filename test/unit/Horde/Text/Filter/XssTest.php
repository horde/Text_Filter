...
class XssTest extends TestCase
{
    ...
    public static function xssProvider()
    {
        ...
        return [
            ...
            ['<<SCRIPT>alert("XSS");//<</SCRIPT>', '<p>alert("XSS");//</p>', ''],
            ...
            ['<STYLE>li {list-style-image: url("javascript:alert(\'XSS\')");}</STYLE><UL><LI>XSS', '<ul><li>XSS</li></ul>'],
            ...
            ['<TABLE><TD BACKGROUND="javascript:alert(\'XSS\')">', '<table><td/></table>'],
            ...
            ['<SCRIPT a=">\'>" SRC="http://ha.ckers.org/xss.js"></SCRIPT>', ''],
            ['<SCRIPT>document.write("<SCRI");</SCRIPT>PT SRC="http://ha.ckers.org/a.js"></SCRIPT>', '<p>PT SRC="http://ha.ckers.org/a.js"&gt;</p>'],
            ...
            ['<a href="data:text/html;base64,PGh0bWw+PGhlYWQ+PHRpdGxlPnRlc3Q8L3RpdGxlPjwvaGVhZD48Ym9keT48c2NyaXB0PmFsZXJ0KCd4c3M6ICcgKyBkb2N1bWVudC5jb29raWUpPC9zY3JpcHQ+PC9ib2R5PjwvaHRtbD4=">Click me</a>', '<a>Click me</a>'],
            ['<body/onload=alert(/xss/)>', ''],
            // PHP 8.4+ libxml handles malformed BODY tag differently, preserving escaped trailing content
            ['<img src=""> <BODY ONLOAD="a();"><SCRIPT>function a(){alert(\'XSS\');}</SCRIPT><"" />', '<img src=""/>', '<img src=""/> &lt;"" /&gt;'],
            ['<img src=\'blank.jpg\'style=\'width:expression(alert("xssed"))\'>', '<img src="blank.jpg"/>'],
            [$framedata, ''],
            ['<svg><a xlink:href="data:text/html,<script>alert(/XSS/)</script>"><rect width="1000" height="1000" fill="white"/></a></svg>', '<svg><a><rect width="1000" height="1000" fill="white"/></a></svg>'],
            ['<math><a xlink:href="data:text/html,<script>alert(/XSS/)</script>">click</a></math>', '<math><a>click</a></math>'],
            ['<form action="data:text/html,<script>alert(/XSS/)</script>"><button></form>', '<form><button/></form>'],
        ];
    }
    ...
    public function testStyleXss()
    {
        ...
        foreach ($tests as $key => $val) {
            $this->assertEquals(
                $val,
                Horde_Text_Filter::filter($key, 'xss', [
                    'strip_styles' => false,
                ])
            );
        }
    ...
    public function testBug9567()
    {
        ...
        $this->assertEquals(
            $text,
            Horde_Text_Filter::filter('<html><body>' . $text . '</body></html>', 'xss', [
                'charset' => 'iso-8859-1',
            ])
        );
        ...
        $this->assertEquals(
            $text,
            Horde_Text_Filter::filter('<html><head><meta http-equiv="Content-Type" content="text/html; charset=iso-8859-1"></head><body>' . $text . '</body></html>', 'xss', [
                'charset' => 'iso-8859-1',
            ])
        );
